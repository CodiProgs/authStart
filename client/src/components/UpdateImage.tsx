        ...
        if (e.target.files) {
            try {
                ...
                response && response.data && setUser({
                    avatar: response.data.updateImage.avatar
                })
            } catch (error) {
                console.log(error)
            }
    ...
    return (
        <div>
            <label htmlFor="fileInput" className='relative group rounded-full'>
                <Image width={1920} height={1080} src={URL_SERVER + user.avatar!} alt="" className='w-[200px] h-[200px] object-cover rounded-full cursor-pointer group-hover:shadow-avatar transition-all duration-300 hover:duration-300' />
